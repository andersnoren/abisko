<?php
/**
 * Title: "Frequently Asked Questions" page layout with hero, questions and answers, and a call to action.
 * Slug: abisko/page-faq
 * Categories: abisko, abisko-page
 */
?>
<!-- wp:group {"align":"wide","style":{"spacing":{"blockGap":"var:preset|spacing|90"}},"layout":{"type":"constrained"}} -->
<div class="wp-block-group alignwide">

	<!-- wp:pattern {"slug":"abisko/hero-cover"} /-->

	<!-- wp:group {"align":"wide","style":{"spacing":{"blockGap":"var:preset|spacing|90"}},"layout":{"type":"constrained"}} -->
	<div class="wp-block-group alignwide">

		<!-- wp:pattern {"slug":"abisko/general-faq-section"} /-->

		<!-- wp:pattern {"slug":"abisko/general-faq-section"} /-->

		<!-- wp:pattern {"slug":"abisko/general-faq-section"} /-->

	</div>
	<!-- /wp:group -->

	<!-- wp:pattern {"slug":"abisko/general-separator-wide"} /-->
	
	<!-- wp:pattern {"slug":"abisko/cta-simple-button"} /-->

</div>
<!-- /wp:group -->